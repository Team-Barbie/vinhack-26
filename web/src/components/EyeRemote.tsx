...
export default function EyeRemote({ eye, root, screenKey }: {
  ...
  useEffect(() => {
    ...
    const mark = (button: HTMLButtonElement, now: number, scroll = true) => {
      ...
      setSelectedLabel(button.dataset.remoteLabel ?? button.getAttribute('aria-label') ?? button.textContent?.trim() ?? 'Selected')
      selectedAt = now
    ...
    const keydown = (e: KeyboardEvent) => {
      if (configuring || e.target instanceof HTMLInputElement || e.target instanceof HTMLSelectElement ||
        (e.target instanceof HTMLElement && e.target.closest('.eye-remote'))) return
    ...
    const tick = () => {
      ...
      setStatus(direction === 'center' ? 'Look left, right, up, or down to move' : `${SYMBOLS[direction]} Hold your look to move ${direction}`)
  ...
  return <>
    <div className="eye-remote remote-bar">
      ...
      {!configuring && <p className="remote-howto"><span><b>1</b> Look left, right, up, or down to move</span><span><b>2</b> Look back to the middle to stop</span><span><b>3</b> Hold a blink (~½ second) to choose</span></p>}
